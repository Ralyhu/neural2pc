...
def train(model, optimizer, A, train_pos_edge_index, train_neg_edge_index):
    model.train()
    optimizer.zero_grad()
    assignments = model()
...
def test(model, A_M, train_pos_edge_index, train_neg_edge_index):
    model.eval()
    with torch.no_grad():
        assignments = model()
...
if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Neural2PC with direct optimization of the polarity objective')
    ...
    output_path = basePath_out + dataset_name + "/direct_e" + str(epochs) + "_lr" + str(lr) + gamma_str_path + "/i_" + str(it) + "/" 
    ...
    try:
        ...
        hidden_size = 64
        model = Direct(N=n, gamma=gamma).to(device)
        ...
        for epoch in range(epochs):
            start_t = time.time()
            loss = train(model, optimizer, A, train_pos_edge_index, train_neg_edge_index)
            training_time += (time.time() - start_t)
            train_loss_list.append(-loss)
            #auc, f1 = test()
            start_t = time.time()
            polarity, counts, s1, s2, dict_x = test(model, A_M, train_pos_edge_index, train_neg_edge_index)
