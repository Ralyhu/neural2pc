...
import torch
import scipy.sparse
#from sgat_k2 import SignedGAT 
from models.sgat import SignedGAT 
...
def train(model, optimizer, x, A, train_pos_edge_index, train_neg_edge_index):
    model.train()
    optimizer.zero_grad()
...
def test(model, x, A_M, train_pos_edge_index, train_neg_edge_index):
    model.eval()
    with torch.no_grad():
        z, assignments = model(x, train_pos_edge_index, train_neg_edge_index)
    return model.test_assignments(assignments, A_M)


if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Neural2PC with SGCN with attention as SGNN model')
    ...
    if dr:
        output_path = basePath_out + dataset_name + "/sgat-dr_l" + str(l) + "_" + str(agg) + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path +   "/i_" + str(it) + "/" 
    else:
        output_path = basePath_out + dataset_name + "/sgat_l" + str(l) + "_" + str(agg)  + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path +   "/i_" + str(it) + "/" 
    ...
    try:
        ...
        if dr:
            model = SignedGAT(in_size, hidden_size, num_layers=l, n_nodes=n, gamma=gamma, lamb=5, discrete_regularizer=wd, aggr=agg).to(device)
        else:
            model = SignedGAT(in_size, hidden_size, num_layers=l, n_nodes=n, gamma=gamma, lamb=5, discrete_regularizer=None, aggr=agg).to(device)
        ...
        for epoch in range(epochs):
            start_t = time.time()
            loss = train(model, optimizer, x, A, train_pos_edge_index, train_neg_edge_index)
            training_time += (time.time() - start_t)
            train_loss_list.append(-loss)
            #auc, f1 = test()
            start_t = time.time()
            polarity, counts, s1, s2, dict_x = test(model, x, A_M, train_pos_edge_index, train_neg_edge_index)
