...
import scipy.sparse 
from models.snea.model import SNEA
from collections import defaultdict
from utility import check_result_KCG
from utility import get_edges_clusters

from models.snea.encoders import LayerEncoder
from models.snea.aggregators import FirstLayerAggregator, NonFirstLayerAggregator
...
def create_spectral_features(in_dim, pos_edge_index, neg_edge_index,
                                 num_nodes=None):
        ...
        svd = TruncatedSVD(n_components=in_dim, n_iter=128)
        svd.fit(A)
        x = svd.components_.T
        import sklearn
...
def train(model, optimizer, x, A, train_pos_edge_index, train_neg_edge_index, A_p, A_n, nodes):
    model.train()
    optimizer.zero_grad()
    z, assignments = model(nodes)
...
def test(model, x, A_M, train_pos_edge_index, train_neg_edge_index, A_p, A_n, nodes):
    model.eval()
    with torch.no_grad():
        z, assignments = model(nodes)
        #z, assignments = model(x, train_pos_edge_index, train_neg_edge_index)
    #return model.test(z, test_pos_edge_index, test_neg_edge_index)
    return model.test_assignments(assignments, A_M), z

if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Neural2PC with SNEA as SGNN model')
    ...
    if dr:
        output_path = basePath_out + dataset_name + "/snea-dr" + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path + "/i_" + str(it) + "/" 
    else:
        output_path = basePath_out + dataset_name + "/snea" + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path +  "/i_" + str(it) + "/" 
    ...
    try:
        ...
        x = create_spectral_features(in_size, train_pos_edge_index, train_neg_edge_index, n)
        #x = A

        nposedges = len(pos_edge_indices_i) / 2
        nnegedges = len(neg_edge_indices_i) / 2
        assert nposedges + nnegedges == len(edges)
        cuda = False
        #features = nn.Embedding(num_nodes, num_feats)
        features = x
        num_feats = in_size

        features = nn.Embedding(n, num_feats)
        features.weight = nn.Parameter(torch.FloatTensor(x), requires_grad=False)

        num_neighbors_sample = None # hard to find def value in the paper (ricontrollare)

        class_weights = calculate_class_weights(n, nposedges, nnegedges, w_no=0.35) # 0.35 is def value
        lambda_structure = 4.0 # default value in the paper
        nodes = list(range(n))
        ...
        if dr:
            model = SNEA(n, final_in_dim, final_out_dim, enc2,
                class_weights, lambda_structure, gamma, discrete_regularizer=wd, hid_dims=hidden_size, cuda_available=cuda).to(device)

        else:
            model = SNEA(n, final_in_dim, final_out_dim, enc2,
                class_weights, lambda_structure, gamma, discrete_regularizer=None, hid_dims=hidden_size, cuda_available=cuda).to(device)
        ...
        for epoch in range(epochs):
            start_t = time.time()
            loss = train(model, optimizer, x, A, train_pos_edge_index, train_neg_edge_index, adj_lists_pos, adj_lists_neg, nodes)
            training_time += (time.time() - start_t)
            train_loss_list.append(-loss)
            #auc, f1 = test()
            start_t = time.time()
            (polarity, counts, s1, s2, dict_x), z = test(model, x, A_M, train_pos_edge_index, train_neg_edge_index, adj_lists_pos, adj_lists_neg, nodes)
