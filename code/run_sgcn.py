...
import torch
import scipy.sparse
from models.sgcn import SignedGCN
...
if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Neural2PC with SGCN as SGNN model')
    ...
    if dr:
        output_path = basePath_out + dataset_name + "/sgcn-dr_l" + str(l) + "_" + str(agg) + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path + "/i_" + str(it) + "/" 
    else:
        output_path = basePath_out + dataset_name + "/sgcn_l" + str(l) + "_" + str(agg)  + "_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path +  "/i_" + str(it) + "/" 
    ...
    try:
        ...
        if dr:
            model = SignedGCN(in_size, hidden_size, num_layers=l, n_nodes=n, gamma=gamma, lamb=5, discrete_regularizer=wd, aggr=agg).to(device)
        else:
            model = SignedGCN(in_size, hidden_size, num_layers=l, n_nodes=n, gamma=gamma, lamb=5, discrete_regularizer=None, aggr=agg).to(device)
        ...
        for epoch in range(epochs):
            ...
            #auc, f1 = test()
            start_t = time.time()
            (polarity, counts, s1, s2, dict_x), z = test(model, x, A_M, train_pos_edge_index, train_neg_edge_index)
