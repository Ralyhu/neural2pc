...
from utility import compute_polarity
...
def test(model, x, A_M, train_pos_edge_index, train_neg_edge_index):
    ...
    #return model.test_assignments_T(assignments, A)

if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Neural2PC with only the NN (a simple MLP) module')
    ...
    output_path = basePath_out + dataset_name + "/mlp_e" + str(epochs) + "_lr" + str(lr) + "_wd" + str(wd) + gamma_str_path + "/i_" + str(it) + "/" 
    ...
    try:
        ...
        hidden_size = 64
        #hidden_size = 128
        model = MLP(in_size, hidden_size, num_layers=2, N=n, gamma=gamma, lamb=5).to(device)
