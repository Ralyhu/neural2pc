...
def compute_stats_assignments(x_list, thresholds, best_threshold):
    pos = [x for x in x_list if x > 0]
    neg = [x for x in x_list if x < 0]
    n_pos = len(pos)
    avg_pos = sum(pos)/n_pos if n_pos > 0 else 0
    max_pos = max(pos) if n_pos > 0 else 0
    min_pos = min(pos) if n_pos > 0 else 0
    n_neg = len(neg)
    avg_neg = sum(neg)/n_neg if n_neg > 0 else 0
    max_neg = max(neg) if n_neg > 0 else 0
    min_neg = min(neg) if n_neg > 0 else 0
    n_ths = len(thresholds)
    avg_ths = sum(thresholds)/n_ths if n_ths > 0 else 0
    max_ths = max(thresholds) if n_ths > 0 else 0
    min_ths = min(thresholds) if n_ths > 0 else 0
    dist_x = {}
    dist_x["avg_pos"] = avg_pos
    dist_x["n_pos"] = n_pos
    dist_x["min_pos"] = min_pos
    dist_x["max_pos"] = max_pos

    dist_x["avg_neg"] = avg_neg
    dist_x["n_neg"] = n_neg
    dist_x["min_neg"] = min_neg
    dist_x["max_neg"] = max_neg

    dist_x["avg_ths"] = avg_ths
    dist_x["n_ths"] = n_ths
    dist_x["min_ths"] = min_ths
    dist_x["max_ths"] = max_ths

    dist_x["best_th"] = best_threshold
...
def round_by_max_obj_one_threshold(v_in, pos, neg, mask, A, N):
    ...
    (v_round, best_th) = (x, x_th) if x_obj > y_obj else (y, y_th)
    return v_round.copy(), best_th
...
def get_edges_clusters(A, membership):
    ...
    for i,j,v in zip(cx.row, cx.col, cx.data):
        if (membership[i] == 1 or membership[i] == 2) and (membership[j] == 1 or membership[j] == 2):
            ci = membership[i]
            cj = membership[j]
            if ci == cj:
                if ci == 1:
