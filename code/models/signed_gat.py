...
class SignedGAT(MessagePassing):
    r"""The signed graph convolutional operator from the `"Signed Graph
    Convolutional Network" - modified with GAT mechanism <https://arxiv.org/abs/1808.06354>`_ paper
    ...
    def __init__(self, in_channels: int, out_channels: int, first_aggr: bool,
        ...
        if first_aggr:
            ...
            self.gat_layer_pos = GATv2Conv(in_channels, in_channels, add_self_loops=self_loops, heads=heads, concat=concat, dropout=dropout)
            self.gat_layer_neg = GATv2Conv(in_channels, in_channels, add_self_loops=self_loops, heads=heads, concat=concat, dropout=dropout)
        else:
    ...
    def reset_parameters(self):
        ...
        self.gat_layer_pos.reset_parameters()
        self.gat_layer_neg.reset_parameters()
    ...
    def forward(self, x: Union[Tensor, PairTensor], pos_edge_index: Adj,
        ...
        if self.first_aggr:
            ...
            out_pos1 = self.gat_layer_pos((x[0][..., :F_in], x[1][..., :F_in]), pos_edge_index)
            out_pos2 = self.gat_layer_neg((x[0][..., F_in:], x[1][..., F_in:]), neg_edge_index)
            ...
            out_neg1 = self.gat_layer_pos((x[0][..., F_in:], x[1][..., F_in:]), pos_edge_index)
            out_neg2 = self.gat_layer_neg((x[0][..., :F_in], x[1][..., :F_in]), neg_edge_index)
